  ...
  const enviarWhatsApp = () => {
    ...
    ["Bienvenida", "Alabanza", "Adoración", "Ofrenda", "Despedida"].forEach(cat => {
      ...
      if (filtradas.length > 0) {
        msg += `*[ ${cat.toUpperCase()} ]*\n`;
        filtradas.forEach(c => msg += `• ${c.titulo} (${c.tono || '?'}) ${c.cantante ? '- ' + c.cantante : ''}\n`);
        msg += `\n`;
      }
    });
  ...
  if (pantalla === 'inicio') {
    return (
      <div style={estilos.fondoInicio}>
        <div style={estilos.capaInst}>🎸 🎹 🥁 🎷</div>
      ...
      <div style={estilos.container}>
        ...
        {pantalla === 'preparar' && (
          <div style={{display:'flex', flexDirection:'column', alignItems:'center'}}>
            <style>{`.react-calendar__tile { color: black !important; font-weight: bold; } .react-calendar { border-radius: 15px; border: none; padding: 10px; }`}</style>
            <div style={{background:'#fff', padding:'15px', borderRadius:'20px', width:'100%', marginBottom:'20px'}}>
        ...
        {pantalla === 'ensayo' && (
          <div>
            <h2 style={{textAlign:'center', color:'#4da6ff'}}>{director || 'PLAN DE CULTO'}</h2>
...
const estilos = {
  fondoInicio: { background: 'linear-gradient(135deg, #f0f9ff 0%, #c0e8ff 100%)', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'fixed', width: '100%', top:0, left:0 },
  capaInst: { position:'absolute', fontSize:'4rem', opacity:0.05, textAlign:'center', width:'100%', pointerEvents:'none' },
  homeContent: { zIndex:1, textAlign:'center', width:'90%', maxWidth:'400px' },
  logoBox: { width:'140px', height:'140px', margin:'20px auto', background:'#fff', borderRadius:'40px', padding:'5px', boxShadow:'0 10px 20px rgba(0,0,0,0.1)' },
  gridBotones: { display:'grid', gridTemplateColumns:'1fr 1fr', gap:'15px' },
  btnHome: { background:'#fff', border:'none', padding:'25px 10px', borderRadius:'20px', fontWeight:'bold', color:'#334155', boxShadow:'0 4px 10px rgba(0,0,0,0.05)' },
  ...
  btnWA: { background:'#25D366', color:'#fff', border:'none', padding:'10px 12px', borderRadius:'10px', fontWeight:'bold', fontSize:'0.8rem' },
  btnBorrar: { background:'#ef4444', color:'#fff', border:'none', padding:'10px 12px', borderRadius:'10px', fontWeight:'bold', fontSize:'0.8rem' },
  inputDir: { width:'100%', padding:'12px', marginTop:'15px', borderRadius:'12px', border:'1px solid #ddd', textAlign:'center', fontWeight:'bold' },
  btnG: { background:'#3b82f6', color:'#fff', border:'none', padding:'10px 15px', borderRadius:'10px', fontWeight:'bold' },
  areaLista: { background:'#0a0a0a', width:'100%', padding:'10px', borderRadius:'20px', border:'1px solid #111' },
  ...
  tag: { background:'#4da6ff', color:'#000', padding:'2px 5px', borderRadius:'4px', fontSize:'0.6rem', marginRight:'5px' },
  miniSelect: { background:'#222', color:'#fff', border:'none', fontSize:'0.7rem', borderRadius:'5px', padding:'5px' },
  ...
  divisor: { margin:'20px 0', color:'#4da6ff', fontWeight:'bold', textAlign:'center' },
  search: { width:'100%', padding:'15px', borderRadius:'15px', background:'#111', border:'1px solid #222', color:'#fff', marginBottom:'15px' },
